...
class Train(object):
    def __init__(self, input_model, input_config, train_iter, test_iter, val_iter, case_iter, log, input_ds,
        ...
        if model_path:
            ...
            self.model_path = 'checkpoint/%s.h' % self.config.model_name
            ...
            for batch_idx, batch_data in enumerate(data_iter):
                ...
                for param in self.model.parameters():
                    if param.grad is not None:
                        param.grad += torch.tensor(np.random.laplace(0, self.config.epsilon / self.config.batch_size, param.grad.shape)).to(self.config.device)
    ...
    def evaluate(self, test=False):
        ...
        storing(ndcg_a,ndcg_c,pre_a,pre_c,ap_a,ap_c,api_loss,category_loss,recall_a,recall_c)
        #---------------------------------------
...
if __name__ == '__main__':
    ...
    train_func.model_path = 'checkpoint\MTFM_secured.h'
    # training
    train_func.train()
